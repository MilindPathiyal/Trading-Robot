...
class Indicators():
    ...
    def rsi(self, period: int, method: str = "wilders") ->  pd.DataFrame:
        ...
        # Define the ewma_up days
        self._frame['ewma_up'] = self._price_groups['up_day'].transform(
            lambda x: np.ewma(span=period).mean()
        )
        
        # Define the ewma_down days
    ...
    def sma(self, period: int) -> pd.DataFrame:
        ...
        # Add the SMA indicator to the data frame
        self._frame[column_name] = self._price_groups['close'].transform(
    ...
    def ema(self, period: int, alpha: float = 0.0) -> pd.DataFrame:
        ...
        # Add the EMA indicator to the data frame
        self._frame[column_name] = self._price_groups['close'].transform(
    ...
    def refresh(self):
        ...
        # Loop through all th stored indicators
    ...
    def check_signals(self) -> Union[pd.DataFrame, None]:
        ...
        signals_df = self._stock_frame._check_signals(indicators=self._indictor_signals)

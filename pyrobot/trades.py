...
class Trade():
    ...
    def modify_side(self, side: Optional[str], order_leg_id: int = 0) -> None:
       ...
       # Set the Order.
        if side:
            self.order['orderLegCollection'][order_leg_id]['instruction'] = side.upper()
        else:
            self.order['orderLegCollection'][order_leg_id]['instruction'] = self.order_instructions[self.enter_or_exit][self.side_opposite]

    
    def add_box_range(self, profit_size: float = 0.00, percentage: bool = False, stop_limit: bool = False) -> None:
        ...
        if not stop_limit:
            self.add_stop_loss(profit_size, percentage)
    ...
    def order_response(self, order_response_dict: dict) -> None:
        self.order_response = order_response_dict
    ...
    def _generate_order_id(self)-> str:
        ...
        if self.order:
            ...
            order_id = order_id.format(
                symbol=self.symbol,
                side=self.side,
                orderenter_or_exit=self.enter_or_exit,
    ...
    def add_leg(self, order_leg_id:int, symbol:str, quantity:int, asset_type:str, sub_asset_type:str=None)->List[dict]:
        ...
        leg={}
        leg['instrument']['symbol'] = symbol
        leg['instrument']['symbol'] = asset_type
        leg['quantity'] = quantity

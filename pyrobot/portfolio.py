...
class Portfolio():
    ...
    def add_position(self, symbol: str, asset_type: str, purchase_date: Optional[str], quantity: int = 0, purchase_price: float = 0.0) -> dict:
        # Create nested dictionary for flexibility
    ...
    def remove_position(self, symbol: str) -> Tuple[bool, str]:
        if symbol in self.positions:
            ...
            return(True, "{Symbol} was successfullly removed.".format(symbol=symbol))
        else:
            return(False, "{Symbol} did not exist in portfolio.".format(symbol=symbol))
